...
class TargetSpace(object):
    ...
    def probe(self, params):
        ...
        # ***************************************************
        # Read the existing TOML data from the file
        """
        with open(Path.TOML_FILE, 'r') as toml_file:
            data = toml.load(toml_file)

        # Modify the specific value
        data['config2']['wind_drag'] = str(round(params['wind_drag'], 2))
        data['config2']['wind_angle'] = str(round(params['wind_angle'], 2))
        data['config2']['horizontal_diff'] = str(round(params['horizontal_diff'], 2))

        # Write the updated data back to the TOML file
        with open(Path.TOML_FILE, 'w') as toml_file:
