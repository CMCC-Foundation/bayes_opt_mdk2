# ---------------------------------------------------------------------
# Bayesian Optimization Workflow for Medslik-II Simulation
# Copyright (c) 2023-2024 CMCC Foundation
# Licensed under The MIT License [see LICENSE for details]
# Written by Igor Ruiz Atake, Marco Mariano De Carlo, Gabriele Accarino
# ---------------------------------------------------------------------
...
from src.service.MDK2DataFormatService import MDK2DataFormatService
from simulation.Medslik_II.functions.medslik_utils import *
from simulation.Medslik_II.scripts.BOF import *
from simulation.Medslik_II.scripts.post_processing import *
from simulation.Medslik_II.scripts.pre_processing import *
from simulation.Medslik_II.scripts.templates import *
...
class Gen_Mdk2_Sim:
    ...
    def __init__(self):
        ...
        self.mdk2_sim_extent_service = MDK2SimExtentService()
        self.mdk2_sim_date_service = MDK2SimDateService()
        self.mdk2_sim_coords_service = MDK2SimCoordsService()
        self.mdk2_data_format_service = MDK2DataFormatService()
        
    def gen_mdk2_sim(self):
        # ================= READ THE CONFIG FILE ================= #
        #config = munch.munchify(toml.load(self.path_controller_instance.get_WORKFLOW_CONFIG()))
        #config = dict(self.mdk2_sim_extent_service.get_sim_extent_params_dict(),
        #              self.mdk2_sim_date_service.get_sim_date_params_dict(),
        #              self.mdk2_sim_coords_service.get_sim_coords_params_dict(),
        #              self.mdk2_data_format_service.get_data_format_params_dict())

        # ================= DATA FORMAT INPUTS ================= #
        type             = self.mdk2_data_format_service.get_data_type()                 ### other options: reanalysis
        time_res         = self.mdk2_data_format_service.get_time_resolution()           ### other options: 1h
        process_files    = eval(self.mdk2_data_format_service.get_process_files())       ### False if data is already present
        SIM_NAME_val     = self.mdk2_sim_extent_service.get_SIM_NAME()
        day_val          = self.mdk2_sim_date_service.get_day()
        month_val        = self.mdk2_sim_date_service.get_month()
        year_val         = self.mdk2_sim_date_service.get_year()
        hour_val         = self.mdk2_sim_date_service.get_hour()
        minutes_val      = self.mdk2_sim_date_service.get_minutes()
        lon_degree_val   = self.mdk2_sim_coords_service.get_lon_degree()
        lat_degree_val   = self.mdk2_sim_coords_service.get_lat_degree()
        sim_lenght_val   = self.mdk2_sim_extent_service.get_sim_lenght()
        duration_val     = self.mdk2_sim_extent_service.get_duration()
        oil_api_val      = self.mdk2_sim_extent_service.get_oil_api()
        oil_volume_val   = self.mdk2_sim_extent_service.get_oil_volume()
        number_slick_val = self.mdk2_sim_extent_service.get_number_slick()
        delta_val        = self.mdk2_sim_coords_service.get_delta()
        
        # ================= MEDSLIK MODEL INPUTS ================= #
        simdir          = f'{self.path_controller_instance.get_MEDSLIK_FOLDER()}/cases/'
        SIM_NAME        = SIM_NAME_val                                                  ### Simulation name - format (string)
        sim_date        = f'{day_val}/{month_val}/20{year_val}'                         ### Simulation start day  - format DD/MM/YYYY (string)
        sim_hour        = f'{hour_val}:{minutes_val}'                                   ### Simulation start hour - format HH:mm (string)
        longitude       = np.float32(lon_degree_val)                                    ### Longitude of Simulation spill location - format Decimal degrees (float)
        latitude        = np.float32(lat_degree_val)                                    ### Latitude of Simulation spill  - format Decimal degrees (float)
        sim_lenght      = int(sim_lenght_val)                                           ### Length of the simulation - format hours (int)
        spill_duration  = int(duration_val)                                             ### Duration of the spill - format hours (int)
        oil_api         = np.float32(oil_api_val)                                       ### Oil API - format (float)
        oil_volume      = np.float32(oil_volume_val)                                    ### Volume of oil in tons - format (float) 
        use_satellite   = True                                                          ### Usage of Satellite imagery to model multiple slicks - True/False
        use_slk_contour = True                                                          ### Usage of slicks countours - True/False
        number_slick    = int(number_slick_val)                                         ### Number of slicks to be simulated - format (int)

        simname = f'MDK_SIM_CASE_{sim_date.replace("/","")}_{sim_hour.replace(":","")}_lenght_{sim_lenght}h_duration_{spill_duration}h_{SIM_NAME}'

        # ================= ENVIRONMENTAL FORCINGS  ================= #
        #Select either a fixed delta for lat and lon or specify the bounding box
        delta           = np.float32(delta_val)              ### Standard delta to collect an area around lat and discharge point - format degrees (float)
        if delta == 0:
            lat_min         = 38.2927818
            lat_max         = 41.8927803
            lon_min         = 14.8626471
        ...
        # check if the MET and OCE folders exists in RUN/TEMP
        os.makedirs(os.path.join(self.path_controller_instance.get_MEDSLIK_RUN(), 'TEMP/MET'), exist_ok=True)
        os.makedirs(os.path.join(self.path_controller_instance.get_MEDSLIK_RUN(), 'TEMP/OCE'), exist_ok=True)
        ...
        if True:
            ...
            if type == 'analysis':
                if loc == 'med' and time_res == 'day':
                    if dt_sim.year < 2021:
                        ...
                        path = f'/data/inputs/METOCEAN/historical/model/ocean/CMS/MedSea/CMCC/analysis/day/'
                ...
                if loc == 'med' and time_res == 'day':
                    if dt_sim.year < 2021:
                        ...
                        path = f'/data/inputs/METOCEAN/historical/model/ocean/CMS/MedSea/CMCC/reanalysis/day/'
            
                elif loc == 'glob':
                    if dt_sim.year > 2010:
                        raise ValueError('Data not available for reanalysis')
                    else:
                        path = f'/data/inputs/METOCEAN/historical/model/ocean/CMS/GlobOce/CMCC/reanalysis/day/'            
            ...
            os.makedirs(f'{simdir}{simname}/', exist_ok=True)
            os.makedirs(f'{simdir}{simname}/oce_files', exist_ok=True)
            os.makedirs(f'{simdir}{simname}/met_files', exist_ok=True)
            os.makedirs(f'{simdir}{simname}/bnc_files', exist_ok=True)
            os.makedirs(f'{simdir}{simname}/xp_files', exist_ok=True)
            os.makedirs(f'{simdir}{simname}/out_files', exist_ok=True)
            os.makedirs(f'{simdir}{simname}/detections', exist_ok=True)
            ...
            ##### SEA / OCEAN ##### 
                for date in date_list:
                    ...
                    for var,varr in zip(['TEMP','RFVL'],['temp','curr']):
                        ...
                        path_f = f'/{path}/{year}/{month}/{filename}'
                        #os.path.join(path,year,month,filename)
                        ...
                        tot.to_netcdf(f'{simdir}{simname}/oce_files/{varr}_{year}{month}{day}_{simname}.nc')
                    
                #opening all files in the directory and concatenating them automatically through open_mfdataset
                concat = xr.open_mfdataset(f'{simdir}{simname}/oce_files/*.nc')
                ...
                for i in range(0,len(concat.time)):            
                    ...
                    with open(f'{simdir}{simname}/oce_files/merc{dt.year-2000:02d}{dt.month:02d}{day:02d}{hour:02d}.mrc', 'w') as f:
                ...
                for date in date_list:
                    ...
                    met.to_netcdf(f'{simdir}{simname}/met_files/{varr}_{year}{month}{day}_{simname}.nc')
                    ...
                    with open(f'{simdir}{simname}/met_files/erai{dt.year-2000:02d}{dt.month:02d}{dt.day:02d}.eri', 'w') as file:
            ...
            #sending curr asc files to TEMP dir to execute medlisk
            subprocess.run([f'cp {simdir}{simname}/oce_files/*.mrc {self.path_controller_instance.get_MEDSLIK_FOLDER()}/MEDSLIK_II_3.01/RUN/TEMP/OCE/'],shell=True)
            
            #sending wind asc files to TEMP dir to execute medlisk
            subprocess.run([f'cp {simdir}{simname}/met_files/*.eri {self.path_controller_instance.get_MEDSLIK_FOLDER()}/MEDSLIK_II_3.01/RUN/TEMP/MET/'],shell=True)
            ...
            #preproc_gebco(f'{Path.MEDSLIK_FOLDER}/data/gebco/GEBCO_2023.nc', grid_string, f'{simdir}{simname}/bnc_files/')
            subprocess.run([f'{sys.executable}', f'{self.path_controller_instance.get_MEDSLIK_FOLDER()}/scripts/pre_processing/preproc_gebco_mdk2.py', 
                            f'{self.path_controller_instance.get_MEDSLIK_FOLDER()}/data/gebco/GEBCO_2023.nc',
                            grid_string, f'{simdir}{simname}/bnc_files/'])

            # gshhs in intermediate resolution
            #preproc_gshhs(f'{Path.MEDSLIK_FOLDER}/data/gshhs/h/GSHHS_h_L1.shp', grid_string, f'{simdir}{simname}/bnc_files/')
            subprocess.run([f'{sys.executable}', f'{self.path_controller_instance.get_MEDSLIK_FOLDER()}/scripts/pre_processing/preproc_gshhs_mdk2.py', 
                            f'{self.path_controller_instance.get_MEDSLIK_FOLDER()}/data/gshhs/h/GSHHS_h_L1.shp',
                            grid_string, f'{simdir}{simname}/bnc_files/'])

            ############################ PREPARING FILES TO RUN THE SIMULATION ############################

            # prepare medslik_II.for and config1.txt
            print('> Preparing medslik_II.for and config1.txt files ...')

            # get dimensions from ncfiles
            for var in ['votemper','thetao','uo','vo']:
                try:
                    my_o = xr.open_dataset(glob(f'{simdir}{simname}/oce_files/*nc')[0]).isel(depth=0,time=0)[var].values.shape
                    found_variable = True
                except:
                    continue
            ...
            for var in ['U10M','u10']:
                try:
                    my_w = xr.open_dataset(glob(f'{simdir}{simname}/met_files/*nc')[0]).isel(time=0)[var].values.shape
                    found_variable = True
                except:
                    continue
                if not found_variable:
            ...
            print('    > Modifying medslik_ii.for ...')

            med_for = f'{simdir}{simname}/xp_files/medslik_II.for'

            subprocess.run([f'cp {self.path_controller_instance.get_MEDSLIK_FOLDER()}/scripts/templates/medslik_II_template.for {med_for}'], shell=True)
            ...
            # # modify config_1.txt
            #print('...config1.txt...')
            print('    > Modifying config1.txt ...')

            config = "LC_CTYPE=C && LANG=C"
            config_file = f'{simdir}{simname}/xp_files/config1.txt'

            subprocess.run([f'cp {self.path_controller_instance.get_MEDSLIK_FOLDER()}/scripts/templates/config1_template_0.txt {config_file}'], shell=True)
            ...
            if use_slk_contour == True:
                slik = 'YES'
                with open(f'{self.path_controller_instance.get_MEDSLIK_FOLDER()}/MEDSLIK_II_3.01/RUN/slick_countour.txt', 'r') as file1:
                    content = file1.read()
                subprocess.run([f'cp {self.path_controller_instance.get_MEDSLIK_FOLDER()}/MEDSLIK_II_3.01/RUN/slick_countour.txt {simdir}{simname}/xp_files/'],shell=True)
            ...
            subprocess.run([f'cp {simdir}{simname}/oce_files/* {self.path_controller_instance.get_MEDSLIK_FOLDER()}/MEDSLIK_II_3.01/METOCE_INP/PREPROC/OCE/'],shell=True)
            subprocess.run([f'cp {simdir}{simname}/oce_files/* {self.path_controller_instance.get_MEDSLIK_FOLDER()}/MEDSLIK_II_3.01/RUN/TEMP/OCE/'],shell=True)

            subprocess.run([f'cp {simdir}{simname}/met_files/* {self.path_controller_instance.get_MEDSLIK_FOLDER()}/MEDSLIK_II_3.01/METOCE_INP/PREPROC/MET/'],shell=True)
            subprocess.run([f'cp {simdir}{simname}/met_files/* {self.path_controller_instance.get_MEDSLIK_FOLDER()}/MEDSLIK_II_3.01/RUN/TEMP/MET/'],shell=True)
            # copy bnc files
            subprocess.run([f'cp {simdir}{simname}/bnc_files/* {self.path_controller_instance.get_MEDSLIK_FOLDER()}/MEDSLIK_II_3.01/DTM_INP/'],shell=True)
            # copy Extract and config files
            subprocess.run([f'cp {simdir}{simname}/xp_files/medslik_II.for {self.path_controller_instance.get_MEDSLIK_FOLDER()}/MEDSLIK_II_3.01/RUN/MODEL_SRC/'],shell=True)
            subprocess.run([f'cp {simdir}{simname}/xp_files/config1.txt {self.path_controller_instance.get_MEDSLIK_FOLDER()}/MEDSLIK_II_3.01/RUN/'],shell=True)
